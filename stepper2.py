...
class Motor(object):
    ...
    def move_steps(self,steps):
        """muove degli step indicati il motore se voglio risultati attendibili 
            devono essere divisibili per 8
        """
        target_step_angle = int(steps/8)
        if(target_step_angle>0):
            self._move_cw(target_step_angle)
        else:
            self._move_acw(target_step_angle)
